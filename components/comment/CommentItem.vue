<template>
  <div class="flex my-4">
    <figure class="shrink-0 mr-3 w-8 h-8 md:w-10 md:h-10">
      <img class="mt-4 rounded-full w-full h-full object-cover"
        :src="`/svg/${data.country ? data.country : 'china'}.svg`" :alt="data.location" height="32" width="32" />
      <figcaption class="text-gray-400 text-xs text-center my-2">{{
        data.country ?
          data.country.replace(data.country.substr(0, 1), data.country.substr(0, 1).toUpperCase()) : 'China'
      }}
      </figcaption>
    </figure>
    <div
      ...
      <div class="flex items-center justify-between mt-2">
        <div class="text-sm text-gray-400 font-semibold">
          <button type="button" @click="isOpen = !isOpen" class="inline-flex items-center focus:outline-none">
        ...
        <div v-if="data.count > 0" class="inline-flex items-center">
          <div class="text-sm text-gray-400 font-semibold">
            {{ data.count }} Replies
            <button type="button" class="ml-4 focus:outline-none" @click="operation(isShow = !isShow, data.code)">{{
              isShow ? '收起评论' : '查看评论'
            }}</button>
          </div>
        </div>
      </div>
      <CommentForm :isShow="isOpen" :post="data.posts" :reply="data.code" @retrieve="retireveReplies" />
      <div v-show="isShow">
  ...
  methods: {
    ...
    retireveReplies(code) {
      if (code && code.length > 0) {
        this.$axios.get(`${SERVER_URL.comment}/${code}/replies`).then(res => {
